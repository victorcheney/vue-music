...
export default {
  ...
  methods: {
    onShortcutTouchStart(e) {
      ...
      this.touch.anchorIndex = anchorIndex // 记录初始位置
      this._scrollTo(anchorIndex)
    },
    onShortcutTouchMove(e) {
      let firstTouch = e.touches[0]
      this.touch.y2 = firstTouch.pageY
      let delta = (this.touch.y2 - this.touch.y1) / ANTHOR_HEIGHT | 0 // 滑动偏移量 取整
      let anchorIndex = parseInt(this.touch.anchorIndex) + delta
      this._scrollTo(anchorIndex)
    },
    scroll(pos) { // 监听scroll事件
      this.scrollY = pos.y
    },
    selectItem(item) {
    ...
    _calculateHeight() { // 计算每一个group的高度
      this.listHeight = []
